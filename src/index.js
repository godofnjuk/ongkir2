...
app.post('/order', async(req, res) => {
  ...
  const parsedData =  JSON.parse(data.data);
  const prettyData = {...data, data: {...parsedData, data: JSON.parse(parsedData.data)}};
  console.log('market id:', JSON.stringify(prettyData.data.instanceId));
...
client.connect();
client.query("insert into ongkir7(marketId,refreshToken) values ('"+instance.instance.instanceId+"','"+refreshToken+"') ON CONFLICT (marketId) DO NOTHING;", (err, res) => {
    console.log(err, res);
    client.end();
  }
);
...
app.listen(port, "0.0.0.0", () => console.log(`My Wix Application ${APP_ID} is listening on port ${port}!`));
