...
class Validate {
  ...
  checkIdentifier(node) {
    ...
    if (ident === 'yield' && this.context.isGenerator) this.fail('yield cannot be an identifier inside of a generator function', node);else if (ident === 'await' && this.context.isAsync) this.fail('await cannot be an identifier inside of an async function', node);else if ((0, _Scanner.isStrictReservedWord)(ident)) this.addStrictError(ident + ' cannot be used as an identifier in strict mode', node);
  }
  ...
  checkForInit(init, iterationType) {
    ...
    if (init.type === 'VariableDeclaration') {
      ...
      if (decl.initializer) {
        ...
        if (iterationType === 'in') this.addStrictError(msg, init);else this.fail(msg);
      }
    } else {
