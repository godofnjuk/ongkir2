...
class Printer {
    ...
    for (let i = 0; i < args.length; ++i) {
      switch (args[i]) {
        ...
        case OUTDENT:
          ...
          break;
        default:
          if (typeof args[i] === 'string') this.output += args[i];else this.printNode(args[i]);
          break;
      }
    }
  }
  ...
  UpdateExpression(node) {
    if (node.prefix) this.write(node.operator, node.expression);else this.write(node.expression, node.operator);
  }
  ...
  MemberExpression(node) {
    ...
    if (node.computed) this.write('[', node.property, ']');else this.write('.', node.property);
  }
  ...
  MethodDefinition(node) {
    ...
    switch (node.kind) {
      case 'generator':
        this.write('*');break;
      case 'async':
        this.write('async ');break;
      case 'async-generator':
        this.write('async *');break;
      case 'get':
        this.write('get ');break;
      case 'set':
        this.write('set ');break;
    }
  ...
  ForOfStatement(node) {
    this.write('for', SPACE, '(', node.left, ' of ', node.right, ')', SPACE, node.body);
  }
  ...
  SwitchCase(node) {
    if (node.test) this.write('case ', node.test, ':', INDENT);else this.write('default:', INDENT);
