...
class Parser {
  ...
  nextToken(context) {
    ...
    while (true) {
      ...
      if (type === 'COMMENT') this.addComment(scanner);else break;
    }
  ...
  nodeStart() {
    ...
    while (true) {
      ...
      if (type === 'COMMENT') this.addComment(this.scanner);else break;
    }
  ...
  popContext(collapse) {
    ...
    if (collapse) context.invalidNodes.forEach(node => parent.invalidNodes.push(node));else this.checkInvalidNodes();
  ...
  PropertyDefinition() {
    ...
    if (this.peek('name') === '...') {
      this.read();
      return new AST.SpreadExpression(this.AssignmentExpression(), start, this.nodeEnd());
    }
  ...
  WithStatement() {
    ...
    let node = new AST.WithStatement(this.Expression(), (this.read(')'), this.Statement()), start, this.nodeEnd());
  ...
  MethodDefinition(name, kind, classKind) {
    ...
    return new AST.MethodDefinition(false, kind, name, params, body, start, this.nodeEnd());
  }
  ...
  ClassDeclaration() {
    ...
    return new AST.ClassDeclaration(ident, base, this.ClassBody(kind), start, this.nodeEnd());
  }
  ...
  ClassExpression() {
    ...
    return new AST.ClassExpression(ident, base, this.ClassBody(kind), start, this.nodeEnd());
  }
  ...
  ExportDeclaration() {
    ...
    switch (this.peek()) {
      ...
      case 'IDENTIFIER':
        if (this.peekLet()) decl = this.LexicalDeclaration();else if (this.peekAsync() === 'function') decl = this.FunctionDeclaration();else return this.ExportDefaultFrom(start);
...
function mixin(target, ...sources) {
  ...
  sources.map(source => source.prototype).forEach(source => ownNames(source).concat(ownSymbols(source)).filter(key => !hasOwn.call(target, key)).forEach(key => Object.defineProperty(target, key, ownDesc(source, key))));
}
